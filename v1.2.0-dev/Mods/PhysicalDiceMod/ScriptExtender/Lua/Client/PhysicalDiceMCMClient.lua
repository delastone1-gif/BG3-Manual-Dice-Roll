...
local function RegisterCallbacks()
    ...
    Log("MCM available! Setting up client-side keybindings...")
        ...
        -- Register Event Button callbacks using MCM.EventButton API
        ...
        local eventButtonSuccess = pcall(function()
            ...
            MCM.EventButton.RegisterCallback({
                ...
                callback = function(buttonId, modUUID)
            ...
            -- Apply Damage button
            MCM.EventButton.RegisterCallback({
                buttonId = "apply_damage_button",
                callback = function(buttonId, modUUID)
                    Log("APPLY DAMAGE BUTTON PRESSED!")
                    local damageValue = GetMCMSetting("current_damage_value") or 40
                    Log("Applying damage value: " .. damageValue)
                    SendCommandToServer("setdamage", damageValue)
                end
            })

            -- Quick Damage 10
            MCM.EventButton.RegisterCallback({
                buttonId = "quick_damage_10",
                callback = function(buttonId, modUUID)
                    Log("Quick Damage 10 button pressed")
                    if IsMCMAvailable() then
                        Mods.BG3MCM.MCMAPI:SetSettingValue("current_damage_value", 10, ModuleUUID)
                    end
                    SendCommandToServer("setdamage", 10)
                end
            })

            -- Quick Damage 40
            MCM.EventButton.RegisterCallback({
                buttonId = "quick_damage_40",
                callback = function(buttonId, modUUID)
                    Log("Quick Damage 40 button pressed")
                    if IsMCMAvailable() then
                        Mods.BG3MCM.MCMAPI:SetSettingValue("current_damage_value", 40, ModuleUUID)
                    end
                    SendCommandToServer("setdamage", 40)
                end
            })

            -- Quick Damage 80
            MCM.EventButton.RegisterCallback({
                buttonId = "quick_damage_80",
                callback = function(buttonId, modUUID)
                    Log("Quick Damage 80 button pressed")
                    if IsMCMAvailable() then
                        Mods.BG3MCM.MCMAPI:SetSettingValue("current_damage_value", 80, ModuleUUID)
                    end
                    SendCommandToServer("setdamage", 80)
                end
            })

            Log("Event button callbacks registered successfully!")
        end)

        if not eventButtonSuccess then
            Log("ERROR: Could not register event button callbacks - MCM.EventButton API not available")
        end
end
